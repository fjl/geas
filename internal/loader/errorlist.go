...
func NewErrorList(maxErrors int) *ErrorList {
	e := &ErrorList{}
	e.SetMaxErrors(maxErrors)
	return e
}

// SetMaxErrors sets the amount of errors that cancels compilation.
func (e *ErrorList) SetMaxErrors(n int) {
	e.maxErrors = max(n, 1)
}
...
func (e *ErrorList) CatchAbort() {
	err := recover()
	if err != nil && err != errCancelCompilation {
		panic(err)
	}
}

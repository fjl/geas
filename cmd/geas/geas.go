...
func usage() {
 ...
 -a: ASSEMBLER (default)
	 ...
	 -no-stackcheck     disable stack checker
	 -stackcheck        (legacy) enable stack checker
...
func assembler(args []string) {
	...
	fs.BoolFunc("stackcheck", "", func(value string) error {
		stackcheck = true
		return nil
	})
	fs.BoolFunc("no-stackcheck", "", func(value string) error {
		stackcheck = false
		return nil
	})
	parseFlags(fs, args)
